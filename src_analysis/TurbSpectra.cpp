                        ...
                        const amrex::Vector< std::string >& var_names)
{
    ...
    // we only need to build the plan once; track whether we did
    heffte_plan fplan;
    int r2c_direction = 0;
#ifdef AMREX_USE_CUDA
    int status = heffte_plan_create_r2c(heffte::backend::cufft, 
#elif AMREX_USE_HIP
    int status = heffte_plan_create_r2c(heffte::backend::rocfft,
#else
    int status = heffte_plan_create_r2c(heffte::backend::fftw,
#endif
    ...
    NULL, r2c_direction, ParallelDescriptor::Communicator(), &fplan);

    if (status != Heffte_SUCCESS) amrex::Abort("Failed at heffte_plan_create() with error code");

    for (int comp=0; comp<ncomp; ++comp) {
        
        using heffte_complex = typename heffte::fft_output<Real>::type;
        heffte_forward_d2z(fplan, variables[local_boxid].dataPtr(comp), 
                           reinterpret_cast<heffte_complex*> spectral_field.dataPtr(comp), 
                           Heffte_SCALE_NONE);
                           ...
                           const amrex::Vector< std::string >& var_names)
{
    ...
    heffte_forward_d2z(fplan, vel[local_boxid].dataPtr(0), 
                       reinterpret_cast<heffte_complex*> spectral_field_T.dataPtr(0), 
                       Heffte_SCALE_NONE);
    heffte_forward_d2z(fplan, vel[local_boxid].dataPtr(1), 
                        reinterpret_cast<heffte_complex*> spectral_field_T.dataPtr(1), 
                        Heffte_SCALE_NONE);
    heffte_forward_d2z(fplan, vel[local_boxid].dataPtr(2), 
                       reinterpret_cast<heffte_complex*> spectral_field_T.dataPtr(2), 
                       Heffte_SCALE_NONE);
    ...
    heffte_plan fplanR;
#ifdef AMREX_USE_CUDA
    status = heffte_plan_create(heffte::backend::cufft, 
#elif AMREX_USE_HIP
    status = heffte_plan_create(heffte::backend::rocfft,
#else
    status = heffte_plan_create(heffte::backend::fftw,
#endif
    {c_local_box.smallEnd(0),c_local_box.smallEnd(1),c_local_box.smallEnd(2)},
    {c_local_box.bigEnd(0)  ,c_local_box.bigEnd(1)  ,c_local_box.bigEnd(2)},
    NULL,
    {local_box.smallEnd(0),local_box.smallEnd(1),local_box.smallEnd(2)},
    {local_box.bigEnd(0)  ,local_box.bigEnd(1)  ,local_box.bigEnd(2)},
    NULL, ParallelDescriptor::Communicator(), NULL, &fplanR);

    if (status != Heffte_SUCCESS) amrex::Abort("Failed at heffte_plan_create() with error code");
    
    // inverse Fourier transform solenoidal and dilatational components 
    heffte_forward_z2d(fplanR, reinterpret_cast<heffte_complex*> spectral_field_S.dataPtr(0), 
                       vel_decomp[local_boxid].dataPtr(0), Heffte_SCALE_NONE);
    heffte_forward_z2d(fplanR, reinterpret_cast<heffte_complex*> spectral_field_S.dataPtr(1), 
                       vel_decomp[local_boxid].dataPtr(1), Heffte_SCALE_NONE);
    heffte_forward_z2d(fplanR, reinterpret_cast<heffte_complex*> spectral_field_S.dataPtr(2), 
                       vel_decomp[local_boxid].dataPtr(2), Heffte_SCALE_NONE);
    heffte_forward_z2d(fplanR, reinterpret_cast<heffte_complex*> spectral_field_D.dataPtr(0), 
                       vel_decomp[local_boxid].dataPtr(3), Heffte_SCALE_NONE);
    heffte_forward_z2d(fplanR, reinterpret_cast<heffte_complex*> spectral_field_D.dataPtr(1), 
                       vel_decomp[local_boxid].dataPtr(4), Heffte_SCALE_NONE);
    heffte_forward_z2d(fplanR, reinterpret_cast<heffte_complex*> spectral_field_D.dataPtr(2), 
                       vel_decomp[local_boxid].dataPtr(5), Heffte_SCALE_NONE);

    heffte_plan_destroy(fplanR);
