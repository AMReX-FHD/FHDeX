                          ...
                          const int& zero_avg)
{
    ...
    Real nsamples_inv = 1.0 / static_cast<Real>(nsamples);
...
void StructFact::IntegratekShells(const int& step, const std::string& name)
{
    ...
    for (MFIter mfi(cov_mag, TilingIfNotGPU()); mfi.isValid(); ++mfi) {
        ...
        amrex::ParallelFor(bx, [=] AMREX_GPU_DEVICE(int i, int j, int k) noexcept {
            ...
            Real dist = static_cast<Real>(ilen * ilen + jlen * jlen + klen * klen);
            dist = std::sqrt(dist);

            if (dist <= center[0] - 0.5) {
                dist = dist + 0.5;
                int cell = static_cast<int>(dist);
                for (int d = 0; d < AMREX_SPACEDIM; ++d) {
    ...
    amrex::ParallelFor(npts, [=] AMREX_GPU_DEVICE(int d) noexcept {
        if (d != 0) {
            phisum_ptr[d] *= 2. * M_PI * (d * dk + 0.5 * dk * dk) / phicnt_ptr[d];
        }
    });
#else
    amrex::ParallelFor(npts, [=] AMREX_GPU_DEVICE(int d) noexcept {
                                        ...
                                        const amrex::Vector<std::string>& names)
{
    ...
    if (names.size() != turbvars)
        amrex::Error("StructFact::IntegratekShellsMisc() requires names to be of the same length as NVAR");
                                ...
                                DistributionMapping& dmap_in)
{
    ...
    VisMF::IO_Buffer io_buffer(amrex::GetIOBufferSize());
