...
void compute_chemistry_source(amrex::Real dt, amrex::Real dV, MultiFab& rho, MultiFab& source, int noffset)
{
    ...
    for (MFIter mfi(rho); mfi.isValid(); ++mfi)
    {
        ...
        amrex::ParallelForRNG(bx, [=] AMREX_GPU_DEVICE(int i, int j, int k, RandomEngine const& engine) noexcept
        {
            ...
            for (int n=0; n<nspecies; n++) n_dens[n] = rho_arr(i,j,k,n+noffset)/m_s[n];
            ...
            for (int n=0; n<nspecies; n++) source_arr(i,j,k,n+noffset) = sourceArr[n];
        });
    }
}
