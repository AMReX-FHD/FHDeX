        ...
        if (advection_type == 1) {
...
#if 1
            // Loop over the cells and compute flux_ines
            amrex::ParallelFor(tbx, tby, tbz,
                [=] AMREX_GPU_DEVICE (int i, int j, int k) {
                    ...
                    xflux(i,j,k,1) += conserved[0]*primitive[1];
                    xflux(i,j,k,2) += conserved[0]*(primitive[1]*primitive[1])+primitive[5];
                    xflux(i,j,k,3) += conserved[0]*primitive[1]*primitive[2];
                    xflux(i,j,k,4) += conserved[0]*primitive[1]*primitive[3];
                    xflux(i,j,k,5) += primitive[1]*conserved[4] + primitive[5]*primitive[1];
                ...
                [=] AMREX_GPU_DEVICE (int i, int j, int k) {
                    ...
                    yflux(i,j,k,1) += conserved[0]*primitive[1];
                    yflux(i,j,k,2) += conserved[0]*(primitive[1]*primitive[1])+primitive[5];
                    yflux(i,j,k,3) += conserved[0]*primitive[1]*primitive[2];
                    yflux(i,j,k,4) += conserved[0]*primitive[1]*primitive[3];
                    yflux(i,j,k,5) += primitive[1]*conserved[4] + primitive[5]*primitive[1];

                    if (algorithm_type_gpu == 2) { // Add advection of concentration
                        for (int n=0; n<nspecies_gpu; ++n) {
                            yflux(i,j,k,5+n) += conserved[5+n]*primitive[1];
                        }
                    }
                },
                ...
                [=] AMREX_GPU_DEVICE (int i, int j, int k) {
                    ...
                    zflux(i,j,k,1) += conserved[0]*primitive[1];
                    zflux(i,j,k,2) += conserved[0]*(primitive[1]*primitive[1])+primitive[5];
                    zflux(i,j,k,3) += conserved[0]*primitive[1]*primitive[2];
                    zflux(i,j,k,4) += conserved[0]*primitive[1]*primitive[3];
                    zflux(i,j,k,5) += primitive[1]*conserved[4] + primitive[5]*primitive[1];

                    if (algorithm_type_gpu == 2) { // Add advection of concentration
                        for (int n=0; n<nspecies_gpu; ++n) {
                            zflux(i,j,k,5+n) += conserved[5+n]*primitive[1];
                        }
                    }
                });
