                                          ...
                                          const DistributionMapping& dm)
{
    ...
    if (lev == 0) {
       ...
       InitSurfaceFFT(ba, dm, Geom(lev));
       //UpdateSurfaceFromFourier(Geom(lev), time);
       ...
       for (MFIter mfi(gmetric); mfi.isValid(); ++mfi)
        {
            ...
            [=] AMREX_GPU_DEVICE(int i, int j, int k)
            {
                init_dk_metric(i,j,k,gmet_arr,gsqr_arr,detg_arr,dhdx_arr,dhdy_arr,dx,problo);
...
AmrCoreAdv::ReadParameters ( amrex::Vector<int>& bc_lo, amrex::Vector<int>& bc_hi)
{
    ...
    {
        ParmParse pp("surface");

        int nkx = pp.countval("kx");
        int nky = pp.countval("ky");
        int namp = pp.countval("amp");
        int nphase = pp.countval("phase");

        int nmodes = std::max({nkx, nky, namp, nphase});
        if (nmodes == 0) {
            nmodes = 1;
        }

        surf_kx.assign(nmodes, 0);
        surf_ky.assign(nmodes, 0);
        surf_amp.assign(nmodes, 0.0);
        surf_phase.assign(nmodes, 0.0);

        if (nkx > 0) { pp.getarr("kx", surf_kx, 0, nkx); }
        if (nky > 0) { pp.getarr("ky", surf_ky, 0, nky); }
        if (namp > 0) { pp.getarr("amp", surf_amp, 0, namp); }
        if (nphase > 0) { pp.getarr("phase", surf_phase, 0, nphase); }
    }
...
void
AmrCoreAdv::InitSurfaceFFT (const amrex::BoxArray& ba,
                            const amrex::DistributionMapping& dm,
                            const amrex::Geometry& geom)
{
    if (!height.isDefined()) {
        height.define(ba, dm, 1, 1);
        dhdx.define(ba, dm, 1, 1);
        dhdy.define(ba, dm, 1, 1);
    }

    if (!surface_fft) {
        surface_fft = std::make_unique<SurfaceFFT>(geom.Domain());
    }
    auto const& spectral_layout = surface_fft->getSpectralDataLayout();
    if (!h_hat.isDefined()) {
        amrex::Print() << " setting up fft for surface height " << std::endl;
        h_hat.define(spectral_layout.first, spectral_layout.second, 1, 0);
        dhdx_hat.define(spectral_layout.first, spectral_layout.second, 1, 0);
        dhdy_hat.define(spectral_layout.first, spectral_layout.second, 1, 0);
    }
    h_hat.setVal(0);
    dhdx_hat.setVal(0);
    dhdy_hat.setVal(0);

        for (MFIter mfi(h_hat); mfi.isValid(); ++mfi) {
              auto const& hb = h_hat[mfi].box();
              auto const& h = h_hat[mfi].array();
              auto const& dhdx = dhdx_hat[mfi].array();
    ...
    stochSurf.define(spectral_layout.first, spectral_layout.second, 2, 0);

    surface_fft->backward(h_hat, height);
    surface_fft->backward(dhdx_hat, dhdx);
    surface_fft->backward(dhdy_hat, dhdy);

    const auto dom = geom.Domain();
    const int nx = dom.length(0);
    const int ny = dom.length(1);
    const amrex::Real inv_norm = 1.0 / (static_cast<amrex::Real>(nx) * static_cast<amrex::Real>(ny));
    height.mult(inv_norm, 0, 1);
    dhdx.mult(inv_norm, 0, 1);
    dhdy.mult(inv_norm, 0, 1);

    height.FillBoundary(geom.periodicity());
    dhdx.FillBoundary(geom.periodicity());
    dhdy.FillBoundary(geom.periodicity());
}

void
AmrCoreAdv::UpdateSurfaceFromFourier (const amrex::Geometry& geom, amrex::Real dt)
{
    if (!surface_fft) { return; }

    const auto dom = geom.Domain();
    const int nx = dom.length(0);
    const int ny = dom.length(1);
    const amrex::Real Lx = geom.ProbHi(0) - geom.ProbLo(0);
    const amrex::Real Ly = geom.ProbHi(1) - geom.ProbLo(1);

    const amrex::Real twopi = 2.0 * 3.14159265358979323846264338327950288;

    for(int n=0; n < 2 ; n++){
       MultiFabFillRandom(stochSurf, n, 1., geom);
    }

    dhdx_hat.setVal(amrex::GpuComplex<amrex::Real>(0.0, 0.0));
    dhdy_hat.setVal(amrex::GpuComplex<amrex::Real>(0.0, 0.0));
    ...
    amrex::Real CAPK = 8;
     ...
     using Complex = GpuComplex<Real>;
         for (MFIter mfi(h_hat); mfi.isValid(); ++mfi) {
              auto const& hb    = h_hat[mfi].box();
              auto const& h_arr = h_hat[mfi].array();
              auto const& xi_arr = stochSurf[mfi].array();
              ParallelFor(hb, [=] AMREX_GPU_DEVICE (int i, int j, int k) noexcept {
                  amrex::Real kx = i;
                  amrex::Real ky = j;
                  if(j > ny/2){
                  ...
                  if(kmag < CAPK && (i*i + j*j != 0) ) {
                     ...
                     amrex::Real Gammak = (kappastar * std::pow(twopi * kmag,3) + sigmastar * twopi*kmag);

                     //h_arr(i,j,k) = std::exp(-Gammak * dt)*h_arr(i,j,k) + std::sqrt(Pik*(1.-std::exp(-2.*Gammak*dt))) * xi_arr(i,j,k);
                     h_arr(i,j,k) = std::exp(-Gammak * dt)*h_arr(i,j,k) + std::sqrt(Pik*(1.-std::exp(-2.*Gammak*dt))) *
                             amrex::GpuComplex<amrex::Real> ( xi_arr(i,j,k,0),xi_arr(i,j,k,1));
 ...
 /*
    for (amrex::MFIter mfi(h_hat); mfi.isValid(); ++mfi) {
        ...
        auto const& hhat_arr = h_hat.array(mfi);
        ...
        [=] AMREX_GPU_DEVICE (int i, int j, int k) noexcept
        {
            if (j > 0 && j < ny/2) {
                const int j2 = ny - j;
                if (j2 >= jlo && j2 <= jhi) {
                    const auto v = hhat_arr(i,j,k,0);
                    hhat_arr(i,j2,k,0) = amrex::GpuComplex<amrex::Real>(v.real(), -v.imag());
                }
            } else if (j == 0 || ((ny % 2) == 0 && j == ny/2)) {
                const auto v = hhat_arr(i,j,k,0);
                hhat_arr(i,j,k,0) = amrex::GpuComplex<amrex::Real>(v.real(), 0.0);
            }
        });
    }
   ...
   */
    ...
    const amrex::GpuComplex<amrex::Real> I(0.0, 1.0);

    for (amrex::MFIter mfi(h_hat); mfi.isValid(); ++mfi) {
        const amrex::Box& bx = mfi.validbox();
        auto const& hhat_arr = h_hat.array(mfi);
        auto const& dhdx_arr = dhdx_hat.array(mfi);
        auto const& dhdy_arr = dhdy_hat.array(mfi);
        amrex::ParallelFor(bx,
        [=] AMREX_GPU_DEVICE (int i, int j, int k) noexcept
        {
            const int ky = (j <= ny/2) ? j : j - ny;
            const amrex::Real kx_val = twopi * static_cast<amrex::Real>(i) / Lx;
            const amrex::Real ky_val = twopi * static_cast<amrex::Real>(ky) / Ly;
            const amrex::GpuComplex<amrex::Real> v = hhat_arr(i,j,k,0);
            dhdx_arr(i,j,k,0) = I * kx_val * v;
            dhdy_arr(i,j,k,0) = I * ky_val * v;
        });
    }
...
AmrCoreAdv::WritePlotFile () const
{
    ...
    for (MFIter mfi(mf[lev]); mfi.isValid(); ++mfi)
    {
            ...
            auto const& height_arr = height.const_array(mfi);
            amrex::ParallelFor(vbx,
            [=] AMREX_GPU_DEVICE(int i, int j, int k)
            {
                amrex::Real measure, nparts;
                ...
                mf_arr(i,j,k,10) = height_arr(i,j,k,0);
    ...
    for (MFIter mfi(mf_nd[nd_lev]); mfi.isValid(); ++mfi)
    {
        ...
        [=] AMREX_GPU_DEVICE(int i, int j, int k)
        {
            ...
            mf_nd_arr(i,j,k,2) = height_arr(i,j,k,0);
            //mf_nd_arr(i,j,k,2) = 0.25*(height_arr(i,j,k,0)+height_arr(i-1,j,k,0)+height_arr(i,j-1,k,0)+height_arr(i-1,j-1,k,0));
        });
    }
