                                          ...
                                          const DistributionMapping& dm)
{
    ...
    if (lev == 0) {
       ...
       for (MFIter mfi(gmetric); mfi.isValid(); ++mfi)
        ...
        surf_area = detg.sum(1) * dx[0] * dx[1];
    ...
    if (lev == 0) {
        ...
        MultiFab::Multiply(volume, detg, 1, 0, 1, 0);
...
//                mf_arr(i,j,k,10) = 3.*sinx*siny;
                mf_arr(i,j,k,10) = det_arr(i,j,k,1);
#endif
...
*/
            mf_nd_arr(i,j,k,2) = 3.*sinx*siny;
        });
    }


    Vector<std::string> varnames = {"phi", "phi0","measure","number","mean_mu","var_mu","var_mu_th","mean_N","var_N","var_N_th","surface"};
