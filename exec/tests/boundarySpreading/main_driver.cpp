...
//    }
 ...
 //   StructFact structFact(bp,dmap,var_names);
    ...
    for(step=1;step<=max_step;++step)
    {
        ...
        if(step == 1)
        {
            //particles.SetPosition(0, 0, prob_hi[0]/2.0 - 16*dx[0], 8*dx[1], prob_hi[2]/4.0);
            particles.SetPosition(1, 0, prob_hi[0]/4.0, prob_hi[1]/2.0, prob_hi[2]/2.0);
        }
        if(step == 2)
        {                        
            particles.SetPosition(0, 0, prob_hi[0]/4.0, 0.000000000001*dx[1], prob_hi[2]/2.0);
        }
        if(step == 3)
        {                        
            particles.SetPosition(0, 0, prob_hi[0]/4.0, 0.000000000001*dx[1], 0.00000000001*dx[2]);
        }
        if(step == 4)
        {                        
            particles.SetPosition(0, 0, prob_hi[0]/2.0 - 0.000000000001*dx[0], 0.000000000001*dx[1], 0.00000000001*dx[2]);
        }
        ...
        if(step == 1)
        {
            Print() << "Expected " << 3.355442828e+16 << std::endl;            
            std::cout << "Proc " << ParallelDescriptor::MyProc() << " force total valid region: " << SumFab(source[0],0, 1) << ", " << SumFab(source[1],0, 1) << ", " << SumFab(source[2],0, 1) << std::endl;
            //std::cout << "Proc " << ParallelDescriptor::MyProc() << " force norm valid region: " << source[0].norm1(0) << ", " << source[1].norm1(0) << std::endl;
            //std::cout << "Force total: " << SumFab(source[0],4, 1) << ", " << SumFab(source[1],ang, 1) << std::endl; 
            //particles.PrintParticles();
 
        }
//        if(step == 2)
//        {
//            Print() << "PRE BC on no slip face - expect half of original value for both no slip and slip\n";
//            std::cout << "Proc " << ParallelDescriptor::MyProc() << " force total valid region: " << SumFab(source[0],0, 1) << ", " << SumFab(source[1],0, 1) << std::endl;
//        }
        ...
        if(step == 2)
        {
            Print() << "POST BC on wall - slip: 3.3554432e+16, 33550, 3.3554432e+16, noslip: 0, 0, 0\n";
            std::cout << "Proc " << ParallelDescriptor::MyProc() << " force total valid region: " << SumFab(source[0],0, 1) << ", " << SumFab(source[1],0, 1) << ", " << SumFab(source[2],0, 1) << std::endl;
            //PrintMF(source[0]);
        }
        if(step == 3)
        {
            Print() << "POST BC on wall crossing periodic boundary - slip: 3.3554432e+16, 33549.5, 5.592405333e+16, noslip: 0, 0, 0\n";
            std::cout << "Proc " << ParallelDescriptor::MyProc() << " force total valid region: " << SumFab(source[0],0, 1) << ", " << SumFab(source[1],0, 1) << ", " << SumFab(source[2],0, 1) << std::endl;
            //PrintMF(source[0]);
        }
        if(step == 4)
        {
            Print() << "POST BC on wall crossing box boundary and periodic boundary - slip: 2.796202667e+16, 16774.75, 2.796202667e+16, noslip: 0, 0, 0\n";
            std::cout << "Proc " << ParallelDescriptor::MyProc() << " force total valid region: " << SumFab(source[0],0, 1) << ", " << SumFab(source[1],0, 1) << ", " << SumFab(source[2],0, 1) << std::endl;
            //PrintMF(source[0]);
           // Print() << "Proc " << ParallelDescriptor::MyProc() << " force norm valid region: " << source[0].norm1(0) << ", " << source[1].norm1(0) << std::endl;
        }
