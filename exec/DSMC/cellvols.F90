...
contains        
  ...
  subroutine get_cell_vols(vols, vlo, vhi, dx, samples)bind(c,name="get_cell_vols")
    ...
    integer :: i, j, k, l
    double precision :: pos(3), vcount, rad
      ...
      do j = vlo(2), vhi(2)
        do i = vlo(1), vhi(1)
          ...
          do l = 1, samples
            ...
            pos(1) = i*dx(1) + get_uniform_func()*dx(1)
            pos(2) = j*dx(1) + get_uniform_func()*dx(2)
            pos(3) = k*dx(1) + get_uniform_func()*dx(3)
