...
//					getSpeciesIndexRet(i_spec,j_spec,&ij_spec);
	                ...
	                if(i_spec<j_spec)
	                {
		                ij_spec = j_spec+nspecies*i_spec;
	                } else
	                {
		                ij_spec = i_spec+nspecies*j_spec;
	                }
...
void FhdParticleContainer::CalcSelections(Real dt)
{
	...
	for(MFIter mfi(mfvrmax); mfi.isValid(); ++mfi)
	{
        ...
        dsmcInterSpecies* interpropertiesTmp = interproperties;
...
//					ij_spec = getSpeciesIndex(i_spec,j_spec);
					...
					crossSection = interpropertiesTmp[ij_spec].csx;
