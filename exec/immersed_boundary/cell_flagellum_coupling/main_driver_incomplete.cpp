...
//#include "StochMomFlux.H"
//#include "hydro_functions.H"
#include "electrostatic.H" //need to comment out once disabled/deleted
...
void main_driver(const char * argv) {
    ...
    // BoxArray for the particles on the cell body
    BoxArray bc;

    // BoxArray electrostatic grid. We probably don't need this
    BoxArray bp;
    ...
    defineFC(umacM, ba, dmap, 1);
    setVal(umacM, 0.);

    std::array< MultiFab, AMREX_SPACEDIM > touched;
    defineFC(touched, ba, dmap, 1);
    setVal(touched, 0.);
    ...
    // Initialize immersed boundary container for flagellum
