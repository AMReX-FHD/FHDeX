...
void mui_fetch(MultiFab& cu, MultiFab& cutemp, MultiFab& prim, const amrex::Real* dx, mui::uniface2d &uniface, const int step)
{
    // assuming the interface is perpendicular to the z-axis
    // and includes cells with the smallest value of z (i.e. k=0)
    ...
    for (MFIter mfi(cu,false); mfi.isValid(); ++mfi)
    {
        ...
        Dim3 hi = ubound(bx);
        const Array4<Real> & cu_fab = cu.array(mfi);
        const Array4<Real> & cutemp_fab = cutemp.array(mfi);
        const Array4<Real> & prim_fab = prim.array(mfi);
        ...
        for (int k = lo.z; k<= hi.z; ++k) {
            for (int j = lo.y; j<= hi.y; ++j) {
                for (int i = lo.x; i<=hi.x; ++i) {
                    cutemp_fab(i,j,k,1) = cu_fab(i,j,k,1)*cu_fab(i,j,k,1);
                    cutemp_fab(i,j,k,2) = cu_fab(i,j,k,2)*cu_fab(i,j,k,2);
                    cutemp_fab(i,j,k,3) = cu_fab(i,j,k,3)*cu_fab(i,j,k,3);
                }
            }
        }
    }
...
void main_driver(const char* argv)
{
    ...
    */
    ...
    MultiFab cutemp(ba,dmap,nvars,ngc);
    ...
    for(step=1;step<=max_step;++step) {
        ...
        mui_fetch(cu, cutemp, prim, dx, uniface, step);
...
*/
           ...
           WriteHorizontalAverage(cutemp,2,1,3,step,geom);
        }
