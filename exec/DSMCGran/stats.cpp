   ...
   const int lev = 0;  
	for (FhdParIter pti(* this, lev); pti.isValid(); ++pti) {
		...
		*/
		...
		for (int k = smallEnd[2]; k <= bigEnd[2]; k++) {
			...
			for (int l=0; l<nspecies; l++) {
				...
				cuInst(i,j,k,cnt+1) = np_spec*moV;         		    	// mass of species l
				cuInst(i,j,k,1)    += np_spec*moV;							// total mass
				...
				for (int m=0; m<np_spec; m++) {
					...
					cuInst(i,j,k,cnt+11) += pow(u,2)+pow(v,2)+pow(w,2);	// gran. temp of spec l
				}
				...
				moV = moV*0.5;
				cuInst(i,j,k,5)  += cuInst(i,j,k,cnt+5)*moV;  // total tau_xx
				cuInst(i,j,k,6)  += cuInst(i,j,k,cnt+6)*moV;	 // total tau_xy
				cuInst(i,j,k,7)  += cuInst(i,j,k,cnt+7)*moV;  // total tau_xz
				cuInst(i,j,k,8)  += cuInst(i,j,k,cnt+8)*moV;  // total tau_yy
				cuInst(i,j,k,9)  += cuInst(i,j,k,cnt+9)*moV;  // total tau_yz
				cuInst(i,j,k,10) += cuInst(i,j,k,cnt+10)*moV; // total tau_zz
				cuInst(i,j,k,11) += cuInst(i,j,k,cnt+11)*moV; // total energy
				cnt += 12;
			}
		...
		*/
		...
		amrex::ParallelFor(tile_box,[=] AMREX_GPU_DEVICE (int i, int j, int k) noexcept {
			covar(i,j,k,0)  = (covar(i,j,k,0)*stepsMinusOne+cuDel(i,j,k,1)*cuDel(i,j,k,1))*osteps;
			covar(i,j,k,1)  = (covar(i,j,k,1)*stepsMinusOne+cuDel(i,j,k,1)*cuDel(i,j,k,2))*osteps;
			covar(i,j,k,2)  = (covar(i,j,k,2)*stepsMinusOne+cuDel(i,j,k,1)*cuDel(i,j,k,3))*osteps;
			covar(i,j,k,3)  = (covar(i,j,k,3)*stepsMinusOne+cuDel(i,j,k,1)*cuDel(i,j,k,4))*osteps;
			covar(i,j,k,4)  = (covar(i,j,k,4)*stepsMinusOne+cuDel(i,j,k,1)*cuDel(i,j,k,11))*osteps;
			covar(i,j,k,5)  = (covar(i,j,k,5)*stepsMinusOne+cuDel(i,j,k,2)*cuDel(i,j,k,2))*osteps;
			covar(i,j,k,6)  = (covar(i,j,k,6)*stepsMinusOne+cuDel(i,j,k,2)*cuDel(i,j,k,3))*osteps;
			covar(i,j,k,7)  = (covar(i,j,k,7)*stepsMinusOne+cuDel(i,j,k,2)*cuDel(i,j,k,4))*osteps;
			covar(i,j,k,8)  = (covar(i,j,k,8)*stepsMinusOne+cuDel(i,j,k,2)*cuDel(i,j,k,11))*osteps;
			covar(i,j,k,9)  = (covar(i,j,k,9)*stepsMinusOne+cuDel(i,j,k,3)*cuDel(i,j,k,3))*osteps;
			covar(i,j,k,10) = (covar(i,j,k,10)*stepsMinusOne+cuDel(i,j,k,3)*cuDel(i,j,k,4))*osteps;
			covar(i,j,k,11) = (covar(i,j,k,11)*stepsMinusOne+cuDel(i,j,k,3)*cuDel(i,j,k,11))*osteps;
			covar(i,j,k,12) = (covar(i,j,k,12)*stepsMinusOne+cuDel(i,j,k,4)*cuDel(i,j,k,4))*osteps;
			covar(i,j,k,13) = (covar(i,j,k,13)*stepsMinusOne+cuDel(i,j,k,4)*cuDel(i,j,k,11))*osteps;
			covar(i,j,k,14) = (covar(i,j,k,14)*stepsMinusOne+cuDel(i,j,k,11)*cuDel(i,j,k,11))*osteps;
		});
	}
}
