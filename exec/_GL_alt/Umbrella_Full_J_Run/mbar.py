...
for filename in files: #for loop in entries of files array 
    ...
    x_kn.append(data[::1]) #skipping every 500 frames. Set to get one position per umbrella state
    N_k.append(len(data[::1])) #append number of position from umbrella. ( set to 1 6/13/19)
