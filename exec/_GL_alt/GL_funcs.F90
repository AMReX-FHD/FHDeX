  ...
  subroutine rk2_stage1(lo,hi, phi, phin, rannums, integral, energy, teng, dx, dt,phi_avg) bind(C,name="rk2_stage1")
    ...
    ! NOTE : This subroutine calls the Stat_Quant subroutine to compute spatial averages of \phi 
      ...
      real(amrex_real) :: func,factor, dele
       ...
       factor = sqrt(2.d0*noise_coef/(dt*dx(1)*dx(2)))  !noise_coef=k_B T

         do  j=lo(2),hi(2)
           do  i=lo(1),hi(1)

             func =  acoef+2.d0*bcoef*phi(i,j)+3.d0*ccoef*phi(i,j)**2+4.d0*dcoef*phi(i,j)**3 ! polynomial part of energy functional
...
!               + diff_coef/2.d0*((phi(i,j)-phi(i-1,j))**2+(phi(i,j)-phi(i,j-1))**2)
        ...
        !    teng = teng *dx(1)*dx(2)
         ...
         do  j=lo(2),hi(2)
           do  i=lo(1),hi(1)
               ...
               phi(i,j)= phin(i,j)  !rename old field as the new field

          enddo
        enddo
        call Stat_Quant(lo,hi, phi,phi_avg) ! compute spatial averages of the field
  ...
  subroutine initphi(lo,hi, phi, dx) bind(C,name="initphi")
    ...
    ! Set to be a  zero-field with a circle of value 1.0
    ...
    !NOTE: As of 7/2019, the initial condition is set to zero at the last line. 
      ...
      integer :: i,j
         ...
         xcen = 0.5d0*(prob_lo(1) + prob_hi(1)) ! compute x coordinate of  center
         ycen = 0.5d0*(prob_lo(2) + prob_hi(2)) ! compute y coordinate of  center
         ...
         do  j=lo(2),hi(2)
           ...
           !  if( (xloc-.5d0)**2 + (yloc-.5d0)**2 .lt. rad**2)then
              if( (xloc-xcen)**2 + (yloc-xcen)**2 .lt. rad**2)then
                 phi(i,j) = 1.d0    !within circle set value to 1
              else
                 phi(i,j) = 0.d0    !set value to 0 outside circle 
              endif


          enddo
        enddo
      phi=phi*0.0d0 ! set initial condition to zero
      ...
      integer :: i,j
         ...
         do  j=lo(2),hi(2)
           do  i=lo(1),hi(1)
              ...
              integral = integral +  (phi(i,j) - phi0)*dx(1)*dx(2)  !use 2D mid-point rule to compute integral 
