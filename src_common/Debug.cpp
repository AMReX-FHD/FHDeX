...
void PrintMFLine (const MultiFab& MF, int comp_lo, int comp_hi, int j, int k)
{
    BL_PROFILE_VAR("PrintMF()",PrintMF);
    ...
    for (int i = 0; i < ba.size(); ++i) {
        if (dm[i] == myProc) {
            ...
            if(j>=(lo[1]-MF.nGrow(1)) && j<=(hi[1]+MF.nGrow(1)) && k>=(lo[2]-MF.nGrow(2)) && k<=(hi[2]+MF.nGrow(2)))
            {
            for (auto ii = lo[0]-MF.nGrow(0); ii <= hi[0]+MF.nGrow(0); ++ii) {
            for (auto comp = comp_lo; comp <= comp_top; ++comp) {
                std::cout << "i: " 
