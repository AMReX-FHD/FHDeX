...
subroutine compute_div(lo, hi, phix, phixlo, phixhi, phiy, phiylo, phiyhi, &
  ...
  real(amrex_real), intent(inout) :: phix(phixlo(1):phixhi(1),phixlo(2):phixhi(2))
  real(amrex_real), intent(inout) :: phiy(phiylo(1):phiyhi(1),phiylo(2):phiyhi(2))
...
#if AMREX_SPACEDIM == 3
subroutine compute_div(lo, hi, velx, velxlo, velxhi, vely, velylo, velyhi, velz, velzlo, velzhi, &
                       div, divlo, divhi, dx, increment) bind(C, name="compute_div")

  integer         , intent(in   ) :: lo(3), hi(3)
  integer         , intent(in   ) :: velxlo(3), velxhi(3), velylo(3), velyhi(3), velzlo(3), velzhi(3)
  ...
  if (increment .eq. 0) then
     ...
     do j = lo(2), hi(2)
     do i = lo(1), hi(1)
        div(i,j,k) = (-velx(i,j,k) + velx(i+1,j,k))*dxInv + &
                     (-vely(i,j,k) + vely(i,j+1,k))*dyInv + &
                     (-velz(i,j,k) + vely(i,j,k+1))*dzInv
     end do
     end do
     end do
  else
     ...
     do j = lo(2), hi(2)
     do i = lo(1), hi(1)
        div(i,j,k) = div(i,j,k) + (-velx(i,j,k) + velx(i+1,j,k))*dxInv + &
                                  (-vely(i,j,k) + vely(i,j+1,k))*dyInv + &
                                  (-velz(i,j,k) + vely(i,j,k+1))*dzInv
     end do
     end do
