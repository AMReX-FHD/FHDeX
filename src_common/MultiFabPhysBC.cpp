...
// BCType::foextrap uses boundary conditions (Neumann) and 1 interior points.
// BCType::ext_dir copies the supplied Dirichlet condition into the ghost cells.
void MultiFabPhysBC(MultiFab& phi, const Geometry& geom, int scomp, int ncomp, int bccomp, const Real& time) {
    ...
    for (MFIter mfi(phi, TilingIfNotGPU()); mfi.isValid(); ++mfi) {
        ...
        if (bx.smallEnd(0) < lo) {
            ...
            // if (bc_lo[0] == BCType::foextrap && bccomp==SPEC_BC_COMP && bc_mass_lo[0] == 4 ) {
            //
           ...
           } else if (bc_lo[0] == BCType::foextrap) {
            ...
            else if (bc_lo[0] == BCType::ext_dir) {
                amrex::ParallelFor(bx, ncomp, [=] AMREX_GPU_DEVICE (int i, int j, int k, int n) noexcept
                {
                    if (i < lo) {
                        Real y = prob_lo[1] + (j+0.5)*dx[1];
        ...
        if (bx.bigEnd(0) > hi) {
            ...
            // if (bc_hi[0] == BCType::foextrap && bccomp==SPEC_BC_COMP && bc_mass_hi[0] == 4 ) {
            ...
            } else if (bc_hi[0] == BCType::foextrap) {
                amrex::ParallelFor(bx, ncomp, [=] AMREX_GPU_DEVICE (int i, int j, int k, int n) noexcept
                {
                    if (i > hi) {
                        Real y = prob_lo[1] + (j+0.5)*dx[1];
            ...
            else if (bc_hi[0] == BCType::ext_dir) {
                amrex::ParallelFor(bx, ncomp, [=] AMREX_GPU_DEVICE (int i, int j, int k, int n) noexcept
                {
                    if (i > hi) {
                        Real y = prob_lo[1] + (j+0.5)*dx[1];
        ...
        if (bx.smallEnd(1) < lo) {
            ...
            } else if (bc_lo[1] == BCType::foextrap) {
                amrex::ParallelFor(bx, ncomp, [=] AMREX_GPU_DEVICE (int i, int j, int k, int n) noexcept
                {
                    if (j < lo) {
            ...
            else if (bc_lo[1] == BCType::ext_dir) {
                amrex::ParallelFor(bx, ncomp, [=] AMREX_GPU_DEVICE (int i, int j, int k, int n) noexcept
                {
                    if (j < lo) {
                        Real x = prob_lo[0] + (i+0.5)*dx[0];
        ...
        if (bx.bigEnd(1) > hi) {
            ...
            } else if (bc_hi[1] == BCType::foextrap) {
                amrex::ParallelFor(bx, ncomp, [=] AMREX_GPU_DEVICE (int i, int j, int k, int n) noexcept
                {
                    if (j > hi) {
                        Real x = prob_lo[0] + (i+0.5)*dx[0];
            ...
            else if (bc_hi[1] == BCType::ext_dir) {
                amrex::ParallelFor(bx, ncomp, [=] AMREX_GPU_DEVICE (int i, int j, int k, int n) noexcept
                {
                    if (j > hi) {
                        Real x = prob_lo[0] + (i+0.5)*dx[0];
        ...
        if (bx.smallEnd(2) < lo) {
            ...
            } else if (bc_lo[2] == BCType::foextrap) {
                amrex::ParallelFor(bx, ncomp, [=] AMREX_GPU_DEVICE (int i, int j, int k, int n) noexcept
                {
                    if (k < lo) {
                        Real x = prob_lo[0] + (i+0.5)*dx[0];
            ...
            else if (bc_lo[2] == BCType::ext_dir) {
                amrex::ParallelFor(bx, ncomp, [=] AMREX_GPU_DEVICE (int i, int j, int k, int n) noexcept
                {
                    if (k < lo) {
                        Real x = prob_lo[0] + (i+0.5)*dx[0];
        ...
        if (bx.bigEnd(2) > hi) {
            ...
            } else if (bc_hi[2] == BCType::foextrap) {
                amrex::ParallelFor(bx, ncomp, [=] AMREX_GPU_DEVICE (int i, int j, int k, int n) noexcept
                {
                    if (k > hi) {
                        Real x = prob_lo[0] + (i+0.5)*dx[0];
            ...
            else if (bc_hi[2] == BCType::ext_dir) {
                amrex::ParallelFor(bx, ncomp, [=] AMREX_GPU_DEVICE (int i, int j, int k, int n) noexcept
                {
                    if (k > hi) {
                        Real x = prob_lo[0] + (i+0.5)*dx[0];
