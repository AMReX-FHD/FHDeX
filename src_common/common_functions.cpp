...
void InitializeCommonNamespace() {
    ...
    initialize_common_namespace(prob_lo.dataPtr(), prob_hi.dataPtr(), n_cells.dataPtr(),
                                ...
                                &fluid_tog, &es_tog, &drag_tog, &move_tog, &rfd_tog, &dry_move_tog, &sr_tog, &graphene_tog, &crange, &thermostat_tog, &images, eamp.dataPtr(), efreq.dataPtr(), ephase.dataPtr(), &plot_ascii, &solve_chem, &diffcoeff, &regrid_int, &do_reflux, &particle_motion);
