...
subroutine advect_2d(time, lo, hi, &
     ...
     &               flxx, fx_lo, fx_hi, &
     &               flxy, fy_lo, fy_hi, &
     &               dx,dt, nu) bind(C, name="advect_2d")
  ...
  double precision, intent(in) :: ptS(pts_lo(1):pts_hi(1),pts_lo(2):pts_hi(2))
...
!     call bl_error("CFL violation. Use smaller adv.cfl.")
  ...
  do    j = lo(2),hi(2)
     do i = lo(1),hi(1)
        if (iface(i,j) .eq. 2) then
        uout(i,j)=uin(i,j)
        else
        uout(i,j) = uin(i,j) + &
             ( (flxx(i,j) - flxx(i+1,j)) * dtdx(1) &
             + (flxy(i,j) - flxy(i,j+1)) * dtdx(2) )+dt*ptS(i,j)
        endif
     enddo
  enddo
