...
!           cy=0.0
!           cz=0.0
!           else if (iface(i,j,k).eq. 1)then
!
!               if (iface(i-1,j,k) .eq. 2) then
!               cx=(con(i+1,j,k)-con(i,j,k))/(2.*dx(1))
!               else if (iface(i+1,j,k) .eq. 2) then
!               cx=(con(i,j,k)-con(i-1,j,k))/(2.*dx(1))
!               else
!               cx=(con(i+1,j,k)-con(i-1,j,k))/(2.*dx(1))
!               end if
!
!               if (iface(i,j-1,k) .eq. 2) then
!               cy=(con(i,j+1,k)-con(i,j,k))/(2.*dx(2))
!               else if (iface(i,j+1,k) .eq. 2) then
!               cy=(con(i,j,k)-con(i,j-1,k))/(2.*dx(2))
!               else
!               cy=(con(i,j+1,k)-con(i,j-1,k))/(2*dx(2))
!               end if
!
!               if (iface(i,j,k-1) .eq. 2) then
!               cz=(con(i,j,k+1)-con(i,j,k))/(2.*dx(3))
!               else if (iface(i,j,k+1) .eq. 2) then
!               cz=(con(i,j,k)-con(i,j,k-1))/(2.*dx(3))
!               else
!               cy=(con(i,j,k+1)-con(i,j,k-1))/(2.*dx(3))
!               end if
!           else
!           cx=(con(i+1,j,k)-con(i-1,j,k))/(2.*dx(1))
!           cy=(con(i,j+1,k)-con(i,j-1,k))/(2.*dx(2))
!           cz=(con(i,j,k+1)-con(i,j,k-1))/(2.*dx(3))
!
!           end if
           if (iface(i,j,k) .lt. 2) then

           ! one sided and centered difference of con
           con_xp=(con(i+1,j,k)-con(i,j,k))/(2*dx(1))
           con_xm=(con(i,j,k)-con(i-1,j,k))/(2*dx(1))
           con_xc=(con_xp+con_xm)

           con_yp=(con(i,j+1,k)-con(i,j,k))/(2*dx(2))
           con_ym=(con(i,j,k)-con(i,j-1,k))/(2*dx(2))
           con_yc=(con_yp+con_ym)

           con_zp=(con(i,j,k+1)-con(i,j,k))/(2*dx(3))
           con_zm=(con(i,j,k)-con(i,j,k-1))/(2*dx(3))
           con_zc=(con_zp+con_zm)
