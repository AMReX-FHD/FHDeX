...
void StochMomFlux::multbyVarSqrtEtaTemp(const MultiFab& eta_cc,
    ...
    for (MFIter mfi(mflux_cc_weighted); mfi.isValid(); ++mfi) {
        ...
        amrex::ParallelFor(bx, AMREX_SPACEDIM, [=] AMREX_GPU_DEVICE (int i, int j, int k, int n) noexcept
        {
        		...
        		if(zero_ice_noise == 1 && eta_cc_fab(i,j,k) > (1+(visc_scale-1.0)*0.5)*visc_coef){
        			noise_switch = 0.0;        		
        		}
        ...
        {
        		Real noise_switch = 1.0;
        		if(zero_ice_noise == 1 && eta_xy_fab(i,j,k) > (1+(visc_scale-1.0)*0.5)*visc_coef){
        			noise_switch = 0.0;        		
        		}
        ...
        {
        		Real noise_switch = 1.0;
        		if(zero_ice_noise == 1 && eta_xz_fab(i,j,k) > (1+(visc_scale-1.0)*0.5)*visc_coef){
        			noise_switch = 0.0;        		
        		}
        ...
        {
        		Real noise_switch = 1.0;
        		if(zero_ice_noise == 1 && eta_yz_fab(i,j,k) > (1+(visc_scale-1.0)*0.5)*visc_coef){
        			noise_switch = 0.0;        		
        		}
