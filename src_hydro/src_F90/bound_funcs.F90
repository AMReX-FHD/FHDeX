...
contains
  ...
  subroutine fab_physbc_domainvel(lo, hi,                   & ! dim(lo) == dim(hi) == 3
    ...
    ! have a hi(n) = n_cells(n) (where n is the direction of the face-centred
    ! quantity) and hi(m) = n_cells(m)-1 for all other direction
    ...
    if(hi(1) .eq. n_cells(1)) then ! upper bound (note: no -1)
       if(bc_hi(1) .eq. 2) then ! no slip thermal
    ...
    if(hi(2) .eq. n_cells(2)) then ! upper bound (note: no -1)
       if(bc_hi(2) .eq. 2) then ! no slip thermal
    ...
    if(hi(3) .eq. n_cells(3)) then ! upper bound (note: no -1)
       if(bc_hi(3) .eq. 2) then ! no slip thermal
